...
export interface OBDData {
  ...
  rpm: number; // rpm
  engineTemp: number; // °C
  engineOilTemp: number; // °C (標準015C + Subaru独自2101)
  atfTemp: number; // °C (独自PID 221017)
  fuelLevel: number; // %
  engineLoad: number; // %
  throttlePosition: number; // %
  timingAdvance: number; // °BTDC
  intakeAirTemp: number; // °C
  manifoldPressure: number; // kPa
  maf: number; // g/s
  controlModuleVoltage: number; // V
  // 1回のPID巡回内で実際に受信できた項目。保存時に未取得項目を除外するために使う。
...
interface MonitoringProfile {
  mode: MonitoringMode;
  // このモードで巡回する PID 群。ラウンドロビンで 1 件ずつ順に要求する。
  pids: string[];
}
...
export class OBDService {
  ...
  private monitoringCallback: ((data: OBDData) => void) | null = null;

  // 1サイクルの中で取得できた値を一時的に保持するキャッシュ。
  // 全PIDが同じタイミングで返るとは限らないため、受信ごとに部分更新する。
  private dataCache: Partial<OBDData> = {};

  // 画面表示へ最後に流したデータ。
  // 今回のループで一部PIDが欠けても、前回値を維持しながら更新するために使う。
  private lastEmittedData: OBDData = {
    speed: 0,
    rpm: 0,
    engineTemp: 0,
    engineOilTemp: 0,
  ...
  private pidCursor = 0;
  private pendingRequest: { finish: () => void } | null = null;
  private requestLoopGeneration = 0;

  // 「>」受信後にだけ次へ進むため、固定待ちは入れずアダプタの処理速度に追従する。
  private readonly interRequestDelayMs = 0;
  // シリアルへの書き込みではなくECU応答を待つため、低速な車両にも余裕を持たせる。
  private readonly responseTimeoutMs = 3000;
  // NO DATA が続く猶予時間。瞬間的な通信揺れで切断扱いにしない。
  private readonly noDataGraceMs = 15000;

  // 監視プロファイル。将来的に ASC カプラー専用モードを増やしやすい構造。
  private readonly profiles: Record<MonitoringMode, MonitoringProfile> = {
    'obd2-standard': {
      mode: 'obd2-standard',
  ...
  private currentMode: MonitoringMode = 'obd2-standard';
  ...
  private cachedVin: string | null = null;
  private connectedPortPath: string | null = null;
  private noDataStreak = 0;
  private lastSuccessfulRxAt = 0;
  private connectionLostNotified = false;
  private onConnectionLostCallback: ((reason: 'port-lost' | 'ignition-off' | 'serial-error') => void) | null = null;

  onConnectionLost(callback: (reason: 'port-lost' | 'ignition-off' | 'serial-error') => void) {
  ...
  private async readVINWithRetry(): Promise<string | null> {
   ...
   */
  startMonitoring(callback: (data: OBDData) => void, mode: MonitoringMode = 'obd2-standard') {
  ...
  getSupportedModes(): MonitoringMode[] {
    return Object.keys(this.profiles) as MonitoringMode[];
  }
   ...
   */
  private async readVIN(command = '09 02'): Promise<string | null> {
  ...
  private publishSnapshot() {
    ...
    const sampledFields: Partial<Record<string, boolean>> = {};
  ...
  /**
    ...
    * 例: 010C = RPM, 010D = 速度。
   */
  private requestPID(pid: string): Promise<void> {
    return new Promise((resolve) => {
      if (!this.port) {
        resolve();
        return;
   ...
   */
  private handleDataLine(line: string) {
  ...
  private parseAtfTemperature(bytes: string[]): number | null {
    const candidates: Array<{ pattern: string[]; valueOffset: number }> = [
  ...
  private decodeTemperaturePayload(payloadBytes: string[], coolantTemp: number | undefined): number | null {
  ...
  private findPatternIndex(haystack: string[], needle: string[]): number {
  ...
  private notifyConnectionLost(reason: 'port-lost' | 'ignition-off' | 'serial-error') {
   ...
   */
  private choosePreferredPort(ports: Array<{ path: string; manufacturer: string; productId?: string; vendorId?: string }>) {
    ...
    const scored = candidates
      .map((port) => {
        ...
        if (path.includes('usbserial')) score += 50;
        if (path.includes('usbmodem')) score += 40;
        if (path.includes('cu.')) score += 20;
        if (path.includes('tty.')) score += 10;

        if (manufacturer.includes('obdlink')) score += 100;
        if (manufacturer.includes('scan') || manufacturer.includes('obd')) score += 60;
        if (manufacturer.includes('ftdi') || manufacturer.includes('silicon labs') || manufacturer.includes('prolific')) score += 30;

        if (port.vendorId) score += 5;
        if (port.productId) score += 5;
