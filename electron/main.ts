...
import { MonitoringMode, OBDData, OBDService } from './obd-service';
import { IoMetricRange, VehicleInfoService } from './vehicle-info-service';
...
let pendingConnectedAt: string | null = null;
let activeDriveVehicle: { vin: string; manufacturer: string; model: string; mode: string; portPath: string | null } | null = null;
let driveLogWriteChain: Promise<void> = Promise.resolve();
...
async function appendDriveLogWithRetry(
  payload: Parameters<VehicleInfoService['appendDriveLog']>[0],
  maxAttempts = 4,
) {
...
function buildFallbackDriveLogVin(portPath: string | null, mode: MonitoringMode, connectedAtIso: string): string {
...
app.on('ready', () => {
  ...
  ipcMain.handle('obd:start-monitoring', async (event, mode?: MonitoringMode) => {
    try {
      if (!activeConnectionId) {
      ...
      obdService.startMonitoring((data) => {
        ...
        if (driveVehicle) {
          const snapshot = data as OBDData;
          const sampledOrNull = (field: keyof OBDData, value: number): number => {
            const sampled = Boolean(snapshot.sampledFields?.[field as string]);
  ...
  ipcMain.handle('obd:supported-modes', async () => {
    return obdService.getSupportedModes();
  });

  ipcMain.handle('vehicle:profile', async () => {
    try {
      const vin = await obdService.getVIN();
  ...
  ipcMain.handle('storage:overview', async () => {
    try {
  ...
  ipcMain.handle('storage:set-log-reservation', async (_event, reservationMb: number) => {
    try {
  ...
  ipcMain.handle('storage:reallocate-reservation', async () => {
    try {
  ...
  ipcMain.handle('storage:io-metrics', async (_event, range: IoMetricRange) => {
    try {
  ...
  ipcMain.handle('vehicles:list-known', async () => {
    return vehicleInfoService.listKnownVehicles();
  });

  ipcMain.handle('vehicles:upsert-known', async (event, payload) => {
    return vehicleInfoService.upsertKnownVehicle(payload);
  });

  ipcMain.handle('vehicles:delete-known', async (event, id: number) => {
    return vehicleInfoService.deleteKnownVehicle(id);
  });

  ipcMain.handle('history:list-connections', async () => {
    return vehicleInfoService.listConnectionHistory();
  });

  ipcMain.handle('drive-logs:list', async (_event, limit?: number) => {
    return vehicleInfoService.listDriveLogs(limit);
  });

  ipcMain.handle('obd:stop-monitoring', async () => {
