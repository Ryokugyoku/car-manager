...
class VehicleInfoService {
    constructor() {
        ...
        this.configuredReservationMb = DEFAULT_LOG_RESERVATION_MB;
        this.lastDriveLogDbSizeBytes = 0;
        this.ioSampleTimer = null;
        this.storageNotice = null;
        this.isDisposing = false;
        this.ioSampleFailureStreak = 0;
        this.driveDbKind = 'drive';
        this.ioDbKind = 'io';
    ...
    setLogReservationMb(reservationMb) {
    ...
    async listIoMetricSamples(range) {
        await this.ensureIoDuckDbReady();
        const now = Date.now();
        let whereSql = '';
        let limitSql = '';
        if (range === '1m') {
            whereSql = `WHERE sampled_timestamp_ms >= ${now - 60000}`;
            limitSql = 'LIMIT 60';
        }
        else if (range === '1h') {
            whereSql = `WHERE sampled_timestamp_ms >= ${now - 3600000}`;
            limitSql = 'LIMIT 3600';
        }
        const sql = `
...
`;
    ...
    startConnectionHistory(payload) {
...
`;
    ...
    finishConnectionHistory(connectionId, reason, disconnectedAt = new Date().toISOString()) {
        const sql = `
...
`;
     ...
     */
    markIncompleteConnectionsAsForceQuit() {
        const sql = `
UPDATE connection_history
...
`;
    ...
    listConnectionHistory(limit = 300) {
...
`;
    ...
    initializeStoragePaths() {
        const rootDir = (0, node_path_1.join)(this.findBaseStorageDirectory(), 'CarManager');
        const dbVolumeDir = (0, node_path_1.join)(rootDir, 'db-volume');
        (0, node_fs_1.mkdirSync)(rootDir, { recursive: true });
        (0, node_fs_1.mkdirSync)(dbVolumeDir, { recursive: true });
        return {
            rootDir,
            dbVolumeDir,
            masterSqlitePath: (0, node_path_1.join)(dbVolumeDir, 'master.sqlite'),
            summaryDuckDbPath: (0, node_path_1.join)(dbVolumeDir, 'drive_logs.duckdb'),
            driveLogReservationPath: (0, node_path_1.join)(dbVolumeDir, 'drive_logs.reserve'),
            ioMetricsDuckDbPath: (0, node_path_1.join)(dbVolumeDir, 'io_metrics.duckdb'),
        };
    }
    findBaseStorageDirectory() {
        ...
        if (process.env.CARMANAGER_TARGET_PLATFORM === 'ios') {
            const iCloudDocuments = (0, node_path_1.join)(home, 'Library', 'Mobile Documents', 'com~apple~CloudDocs', 'Documents');
            if ((0, node_fs_1.existsSync)(iCloudDocuments)) {
        ...
        if (currentPlatform === 'win32' || currentPlatform === 'darwin' || currentPlatform === 'linux') {
            return (0, node_path_1.join)(home, 'Documents');
        }
        return home;
    }
    findOneDriveDirectory(home) {
        const directCandidates = [
            (0, node_path_1.join)(home, 'OneDrive'),
            (0, node_path_1.join)(home, 'OneDrive - Personal'),
            (0, node_path_1.join)(home, 'OneDrive - Business'),
        ];
        ...
        try {
            const children = (0, node_fs_1.readdirSync)(home, { withFileTypes: true })
                .filter((entry) => entry.isDirectory() && entry.name.toLowerCase().startsWith('onedrive'))
                .map((entry) => (0, node_path_1.join)(home, entry.name))
                .sort();
...
`;
        ...
        if (!sqliteResult.ok) {
            const fallbackSchemaPath = (0, node_path_1.join)(this.paths.rootDir, 'master-schema.sql');
            if (!(0, node_fs_1.existsSync)(fallbackSchemaPath)) {
                (0, node_fs_1.writeFileSync)(fallbackSchemaPath, fullSql, { encoding: 'utf8' });
            }
        }
    }
    buildSeedRows() {
    ...
    ensureDriveLogReservation(reservationBytes) {
        ...
        try {
            if ((0, node_fs_1.existsSync)(reservePath)) {
                const currentSize = (0, node_fs_1.statSync)(reservePath).size;
                if (currentSize === reservationBytes) {
                    return { success: true };
                }
                (0, node_fs_1.unlinkSync)(reservePath);
            }
        }
        catch (error) {
        ...
        const result = (0, node_child_process_1.spawnSync)('mkfile', [reserveSizeArg, reservePath], { encoding: 'utf8' });
        if (result.status !== 0 || !(0, node_fs_1.existsSync)(reservePath) || this.getFileSizeBytes(reservePath) !== reservationBytes) {
            return {
                success: false,
    ...
    async ensureLogCapacity(requiredBytes) {
        const reservationBytes = this.configuredReservationMb * BYTES_PER_MB;
    ...
    tryAutoExpandDbVolume(requiredTotalBytes) {
...
`, connection);
        ...
        const countRows = await this.allDuckDb(`SELECT COUNT(*) AS cnt FROM drive_logs WHERE SUBSTR(sample_at, 1, 10)='${escapedDay}';`, connection);
    ...
    getCurrentDbVolumeUsedBytes() {
        return (this.getFileSizeBytes(this.paths.masterSqlitePath)
            + this.getFileSizeBytes(this.paths.summaryDuckDbPath)
            + this.getFileSizeBytes(this.paths.ioMetricsDuckDbPath));
    }
    async ensureDuckDbReady() {
...
`, connection);
    ...
    async ensureDriveLogsColumns(connection) {
        ...
        for (const column of requiredColumns) {
            ...
            await this.runDuckDb(`ALTER TABLE drive_logs ADD COLUMN ${column.name} ${column.type};`, connection, this.driveDbKind);
        }
    }
    resetDuckDbConnection() {
        this.closeDuckDbConnection(this.duckDbConnection);
        this.closeDuckDbDatabase(this.duckDb);
        this.duckDb = null;
        this.duckDbConnection = null;
...
`, connection, this.ioDbKind);
    ...
    async recordIoMetricSampleSafely() {
        if (this.isDisposing) {
            return;
        }
        try {
...
`;
    ...
    saveConfiguredReservationMb(valueMb) {
...
`;
    ...
    runDuckDb(sql, connection = this.getDuckDbConnection(), dbKind = this.driveDbKind) {
            ...
            connection.run(sql, (error) => {
                if (error) {
                    ...
                    try {
                        ...
                        retryConnection.run(sql, (retryError) => {
                            if (retryError) {
                                reject(retryError);
                                return;
                            }
    ...
    allDuckDb(sql, connection = this.getDuckDbConnection(), dbKind = this.driveDbKind) {
        return new Promise((resolve, reject) => {
            connection.all(sql, (error, rows) => {
                if (error) {
                    ...
                    try {
                        const retryConnection = this.resetAndReconnect(dbKind);
                        retryConnection.all(sql, (retryError, retryRows) => {
                            if (retryError) {
                                reject(retryError);
                                return;
                            }
...
`;
    ...
    getFileSizeBytes(filePath) {
        try {
            return (0, node_fs_1.existsSync)(filePath) ? (0, node_fs_1.statSync)(filePath).size : 0;
        }
        catch {
            return 0;
        }
    }
    getVolumeStats(rootPath) {
        try {
            const stats = (0, node_fs_1.statfsSync)(rootPath);
    ...
    toPercent(sizeBytes, capacityBytes) {
        if (capacityBytes <= 0) {
            return 0;
        }
        return Math.max(0, Math.min(100, (sizeBytes / capacityBytes) * 100));
    }
    execute(sql, extraArgs = []) {
        const args = [...extraArgs, this.paths.masterSqlitePath, sql];
        const result = (0, node_child_process_1.spawnSync)('sqlite3', args, { encoding: 'utf8' });
